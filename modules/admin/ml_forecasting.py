...
def show_ml_forecasting(df_orders, df_items, df_products, df_customers, df_vendors):
    ...
    valid_items = df_items[df_items["order_id"].isin(valid_orders["order_id"])] if not valid_orders.empty else pd.DataFrame()
    ...
    if not valid_orders.empty:
        ...
        daily_sales = pd.DataFrame(columns=["date_only", "total_amount", "order_id"])
    ...
    pred_revenue = max(0.0, float((future_indices * slope_rev + intercept_rev).sum()))
    ...
    pred_prod_demand = int(valid_items["quantity"].sum() * 1.15) if not valid_items.empty and "quantity" in valid_items.columns else 150
    pred_active_vendors = int(len(df_vendors) * 1.05) if not df_vendors.empty else 5
    ...
    # Dynamic projection month generation based on max date
    if not df_orders.empty:
        last_month = pd.Period(df_orders["created_at"].max(), freq='M')
        future_months = [(last_month + i).strftime('%Y-%m') for i in range(1, 4)]
    else:
        future_months = ["2026-08", "2026-09", "2026-10"]

    future_sales_est = [
        monthly_sales["total_amount"].iloc[-1] * 1.05 if not monthly_sales.empty else 12000.0, 
    ...
    future_orders_est = [
        int(monthly_orders["order_id"].iloc[-1] * 1.04) if not monthly_orders.empty else 120,
    ...
    with col1:
        ...
        act_vs_pred = pd.DataFrame({
            ...
            "Predicted": [None]*(max(0, len(monthly_sales)-1)) + [monthly_sales["total_amount"].iloc[-1] if not monthly_sales.empty else 10000] + future_sales_est
        })
    ...
    with col3:
        ...
        if "category" in df_products.columns and "current_stock" in df_products.columns:
            ...
            st.info("Product demand parameters missing.")

        # Chart 7: Vendor Performance Forecast – Horizontal Bar Chart
        ...
        if not df_vendors.empty:
            ...
            if "fulfillment_rate" in vend_perf.columns:
                ...
                base_rate = vend_perf["rating"] * 20.0
            else:
    ...
    with col4:
        ...
        if "name" in df_products.columns:
            ...
            thresh = prod_demand["low_stock_threshold"] if "low_stock_threshold" in prod_demand.columns else 10
            prod_demand["Replenishment Units Required"] = (thresh * 1.5).astype(int)
        ...
        if not valid_items.empty and "product_id" in valid_items.columns:
            ...
            if "price_per_unit" in df_merged_full.columns:
                ...
                df_merged_full["total_amount"] = df_merged_full["quantity"] * 10.0

            if "category" in df_merged_full.columns:
                ...
                st.info("Category breakdown missing in merged orders.")
        else:
            st.info("No transaction item history found.")
    ...
    with col5:
        ...
        categories_list = df_products["category"].unique()[:4].tolist() if "category" in df_products.columns and not df_products.empty else ["Electronics", "Fashion", "Home", "Sports"]
